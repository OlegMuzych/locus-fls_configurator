...
export default class MyYModemEsp32 {
    port = 0;
    file = 0;
    port = serialPort;
    consoleOutput = () => {
            ...
            this.port.onData( async  (data) => {
                if (data.length <= 2) {
                    for (let x = 0; x < data.length; x++) {
                        for (let key in MyYModemEsp32) {
                            if (data[x] === MyYModemEsp32[key]) {
    ...
    async #sendStartFrame(name, fileSize) {
        ...
        for(let errorCount = 0; errorCount < MyYModemEsp32.countError; errorCount ++){
            resp = await this.#portWrite(data);
            if (resp[0] == MyYModemEsp32.ack) {
                return resp;
                break;
            }
        }
    ...
    async #sendData(file) {
        ...
        while (!end) {
            this.seq ++;
            let lower = offset * MyYModemEsp32.stx_pocket_len;
            let higher = ((offset + 1) * MyYModemEsp32.stx_pocket_len);
    ...
    #sendEndFrame() {
        ...
        return new Promise(async (resolve, reject)=>{
            for (;;){
                let resp = await this.#portWrite(new Uint8Array([MyYModemEsp32.eot]));
                if(resp[0] == MyYModemEsp32.ack){
                    break;
                }
            }
    ...
    #createSTXFrame(data) {
        ...
        let bufferNull = createArray(0x00, MyYModemEsp32.stx_pocket_len - data.length);
        let buff = Buffer.concat([data, bufferNull], MyYModemEsp32.stx_pocket_len);

        let seqchr = new Uint8Array([this.seq & 0xFF]);
        let seqchr_neg = new Uint8Array([(-this.seq - 1) & 0xFF]);
        let crc16 = mycrc(buff);
        let packet = Buffer.concat([(new Uint8Array([MyYModemEsp32.stx])), seqchr, seqchr_neg, buff, crc16]);
        if (packet.length != MyYModemEsp32.expected_stx_packet_len) {
    ...
    #createSOHFrame(data) {
        ...
        let bufferNull = createArray(0x00, MyYModemEsp32.soh_pocket_len - data.length);
        let buff = Buffer.concat([data, bufferNull], MyYModemEsp32.soh_pocket_len);

        let seqchr = new Uint8Array([this.seq & 0xFF]);
        let seqchr_neg = new Uint8Array([(-this.seq - 1) & 0xFF]);
        let crc16 = mycrc(buff);
        let packet = Buffer.concat([(new Uint8Array([MyYModemEsp32.soh])), seqchr, seqchr_neg, buff, crc16]);
        if (packet.length != MyYModemEsp32.expected_soh_packet_len) {
            throw('Packet length is wrong!');
        }
        return packet;
    }
...
MyYModemEsp32.soh = 1;     // 128 byte blocks
MyYModemEsp32.stx = 2;     // 1K blocks
MyYModemEsp32.eot = 4;
MyYModemEsp32.ack = 6;
MyYModemEsp32.nak = 0x15;
MyYModemEsp32.ca = 0x18;    // 24
MyYModemEsp32.c = 0x43;  // 67
MyYModemEsp32.abort1 = 0x41; // 65
MyYModemEsp32.abort2 = 0x61; // 97

MyYModemEsp32.countError = 3; //3

// 1K blocks does not seem to work
MyYModemEsp32.stx_pocket_len = 1024;//
MyYModemEsp32.soh_pocket_len = 128;//
MyYModemEsp32.expected_stx_packet_len = MyYModemEsp32.stx_pocket_len + 5;
MyYModemEsp32.expected_soh_packet_len = MyYModemEsp32.soh_pocket_len + 5;
